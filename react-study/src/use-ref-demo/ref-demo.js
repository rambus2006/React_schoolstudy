...
const RefDemo = function(props) {
    ...
    const inputEl = useRef() //초기값을 안준건 dom요소를 저장하기 위한 것 
    const canvasEl = useRef()
    //inputEl.current ==> <input typpe="text"/>
    //canvasEl.current ==> <input


    // 그냥 일반적인 값을 저장하기 위해서도 사용 가능
    // (단, 화면에 보여주어야 할 상태값을 저장할 경우에는 useState를 사용해야 함)
    const value = useRef(1) // 초기값 전달 가능 ->객체나 숫자를 저장하기 위한 값들 
    //value.current 속성 안에 초기값1이 저장된다. 
    ...
    return (
        <>
            ...
            {/* ref속성에 inputEl연결, input 한 값은 inputEl.current  */}
            ...
            <button onClick={() => {
                ...
                //input한 값에 focus()가 된다. 
                inputEl.current.focus()
            }}>Focus input</button>
            <hr />
            {/* ref 속성에 앞서 생성한 ref 객체를 전달하는 방식으로 DOM 요소를 저장 */}
            {/* canvas는 js 코드로만 할 수 있어서 코드가 길다.(DOM요소에 저장) */}
